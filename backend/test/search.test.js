...
test('resolveProvider prefers an explicit setting, then the first configured key', () => {
  ...
  assert.equal(resolveProvider({ OPENROUTER_API_KEY: 'k', BRAVE_SEARCH_API_KEY: 'b' }), 'brave');
...
test('searchWeb reads OpenRouter web-plugin citations', async () => {
  clearSearchCache();
  const fetchImpl = async () => new Response(JSON.stringify({
    choices: [{
      message: {
        content: 'Two stores list it.',
        annotations: [
          { type: 'url_citation', url_citation: { url: 'https://a.example.com/p', title: 'Store A', content: '$199.00 in stock' } },
          { type: 'url_citation', url_citation: { url: 'https://b.example.com/p', title: 'Store B', content: '$210.00' } }
        ]
      }
    }]
  }), { headers: { 'Content-Type': 'application/json' } });

  const result = await searchWeb({ query: 'nova earbuds' }, { env: { OPENROUTER_API_KEY: 'k' }, fetchImpl });
  assert.equal(result.provider, 'openrouter');
  assert.equal(result.results.length, 2);
  assert.equal(result.results[0].price, 199);
  assert.equal(result.providerSummary, 'Two stores list it.');
});
